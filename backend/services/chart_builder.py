...
def _build_calibers(plan: AnalysisPlan) -> List[str]:
    caliber = plan.statistical_caliber
    calibers = [
        caliber.description,
        f"去重方式：{caliber.dedup_method}",
        f"聚合粒度：{caliber.time_granularity}",
    ]
...
def build(plan: AnalysisPlan, data: list[dict]) -> ChartConfig:
    ...
    if viz_type in ("gauge", "funnel_chart") and data:
        row = data[0]
